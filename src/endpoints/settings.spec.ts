...
describe("endpoints/settings.ts", () => {
    describe("SettingsEndpoints", () => {
        describe("createSettingsEndpoint(client)", () => {
            ...
            beforeEach(() => {
                testRequestHandler = SettingsEndpoints.createSettingsEndpoint(stubInstance as any);
            });
            afterEach("test and reset stubs", () => {
                expect(stubInstance.getSettings.callCount).to.equal(1);
                sandbox.reset();
            });
            after(() => {
                sandbox.restore();
            ...
            it("should pass on the provided parameters", () => {
                stubInstance.getSettings.resolves(methodStubResponse);
                return testRequestHandler(req, responseObj, nextObj)
                    .then(() => {
                        expect(stubInstance.getSettings.callCount).to.equal(1);
                        expect(stubInstance.getSettings.getCall(0).args).to.deep.equal([]);
                        ...
                        expect(setHeaderStub.getCall(0).args).to.deep
                            .equal(["Cache-Control", "public, max-age=86400"]);
                    });
            });
            it("should fail correctly inner methods correclty", () => {
                stubInstance.getSettings.rejects(methodStubResponse);
                return testRequestHandler(req, responseObj, nextObj)
                    .then(() => {
                        expect(stubInstance.getSettings.callCount).to.equal(1);
                        expect(stubInstance.getSettings.getCall(0).args).to.deep.equal([]);
